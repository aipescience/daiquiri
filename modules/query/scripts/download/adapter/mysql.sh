...
# dump the table
if [ -z "$socket" ]; then
    ${binPath}/mysqldump -h$host -P$port -u$username -p$password $dbname $table > $file 2>> $path/$fileName.err
else
    ${binPath}/mysqldump --socket=$socket -u$username -p$password $dbname $table > $file 2>> $path/$fileName.err
fi
